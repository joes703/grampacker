...
- **Formula-injection handling.** On export: any cell starting with `=`, `+`, `-`, `@`, tab, or CR is prefixed with a single apostrophe to prevent formula evaluation in Excel/Sheets/Numbers. On import: leading apostrophes are preserved as-is, since third-party CSV tools may emit them legitimately and stripping would mangle those imports.

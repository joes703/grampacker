...
// ── Single-row update with cross-cache fan-out ────────────────────────────────
//
// Variant of makeOptimisticUpdate for the case where the primary cache's row
// is also embedded inside rows of OTHER caches. grampacker's canonical case:
// gear_items writes whose changed fields are part of the gear_item join
// projected into list_items (name, description, weight_grams, category_id,
// status — see GEAR_ITEM_AUTH_SELECT in src/lib/queries/projections.ts).
// Without a fan-out, the primary cache (`['gear-items']`) updates immediately
// but every open list view continues to render the stale embedded copy until
// the settled refetch resolves.
//
// The helper writes the patched primary row into the primary cache AND walks
// every active query whose key starts with `fanoutQueryKeyPrefix`, applies
// `applyJoined` to the rows that match `matchJoined`, and snapshots only the
// caches it actually touched so onError rolls back the exact caches it wrote
// to. Settled invalidates the primary key plus only the joined keys that were
// touched (matching the editItem hand-roll this helper replaces — narrow
// invalidation, not blanket).
//
// onMutate is synchronous (NOT `async` + `await`). Same rule as the rest of
// this module: the cache write must land in the same React tick as the
// triggering event so any concurrent CSS transition animates against the
// correct final state. See the "Why onMutate is sync" note on
// makeOptimisticReorder for the full rationale.
//
// Scope today: update-only. Bulk update with fan-out, delete with fan-out,
// and parent-table writes that null-cascade into the embedded projection
// (e.g. deleting a category SETs NULL on gear_items.category_id, which is
// embedded in list_items) are deliberate follow-ups, kept separate so the
// shape proves stable before generalizing.
export function makeOptimisticUpdateWithFanout<
  TPrimary extends { id: string },
  TJoined,
  TInput,
>(opts: CommonOpts & {
  // Prefix-match key for the joined caches. Every active query whose key
  // begins with this array is considered; e.g. `['list-items']` matches
  // `['list-items', listIdA]`, `['list-items', listIdB]`, etc. This is the
  // PostgREST-join's "other side": the cache that EMBEDS rows from the
  // primary cache via a join projection.
  fanoutQueryKeyPrefix: QueryKey
  id: (input: TInput) => string
  // How to apply the patch to the primary row.
  applyPrimary: (item: TPrimary, input: TInput) => TPrimary
  // Does this joined row reference the primary row being patched? Typically
  // `joined.<fk>_id === id(input)`. Determines which rows in each joined
  // cache receive the embedded-patch write.
  matchJoined: (joined: TJoined, input: TInput) => boolean
  // How to patch the joined row's embedded copy. Typically
  // `{ ...joined, <embedField>: { ...joined.<embedField>, ...patch } }`.
  applyJoined: (joined: TJoined, input: TInput) => TJoined
}) {
  const { qc, queryKey, fanoutQueryKeyPrefix, id, applyPrimary, matchJoined, applyJoined } = opts
  type FanoutSnapshot = { key: QueryKey; data: TJoined[] | undefined }
  type Ctx = { previous: TPrimary[] | undefined; fanoutSnapshots: FanoutSnapshot[] }
  return {
    onMutate: (input: TInput): Ctx => {
      qc.cancelQueries({ queryKey })
      const previous = qc.getQueryData<TPrimary[]>(queryKey)
      const targetId = id(input)
      qc.setQueryData<TPrimary[]>(queryKey, (curr) => {
        if (!curr) return curr
        return curr.map((item) => (item.id === targetId ? applyPrimary(item, input) : item))
      })

      // Find every active joined query whose data actually contains a
      // matching row. Skipping unmatched queries keeps the snapshot set
      // (and the settled-time invalidation) tight: a list with no items
      // referencing the patched primary row is not touched and not
      // invalidated.
      const fanoutSnapshots: FanoutSnapshot[] = []
      const candidates = qc.getQueryCache().findAll({ queryKey: fanoutQueryKeyPrefix })
      for (const q of candidates) {
        const data = q.state.data as TJoined[] | undefined
        if (!data || !data.some((row) => matchJoined(row, input))) continue
        const key = q.queryKey
        qc.cancelQueries({ queryKey: key })
        fanoutSnapshots.push({ key, data })
        qc.setQueryData<TJoined[]>(key, (curr) =>
          curr ? curr.map((row) => (matchJoined(row, input) ? applyJoined(row, input) : row)) : curr,
        )
      }

      return { previous, fanoutSnapshots }
    },
    onError: (_err: unknown, _vars: TInput, ctx: Ctx | undefined) => {
      if (!ctx) return
      if (ctx.previous) qc.setQueryData(queryKey, ctx.previous)
      for (const { key, data } of ctx.fanoutSnapshots) {
        qc.setQueryData(key, data)
      }
      if (opts.errorToast) showToast(opts.errorToast, { type: 'error' })
    },
    onSettled: (_data: unknown, _err: unknown, _vars: TInput, ctx: Ctx | undefined) => {
      qc.invalidateQueries({ queryKey })
      if (ctx) {
        for (const { key } of ctx.fanoutSnapshots) {

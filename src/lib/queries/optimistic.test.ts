...
describe('makeOptimisticUpdateWithFanout', () => {
  // Models the gear_items -> list_items join: a primary cache of Gear,
  // and one or more list-items caches whose rows embed a copy of the
  // gear row via `gear_item`. The helper must patch the primary row in
  // ['gear'] AND patch the embedded copy in every ['list-items', *]
  // cache that references it.
  type Gear = { id: string; name: string; weight_grams: number }
  type ListItem = {
    id: string
    gear_item_id: string
    quantity: number
  ...
  type Patch = { id: string; patch: Partial<Gear> }

  function seed(): { qc: QueryClient; gearKey: readonly ['gear']; listKeyA: readonly ['list-items', 'A']; listKeyB: readonly ['list-items', 'B']; listKeyC: readonly ['list-items', 'C'] } {
    const qc = new QueryClient()
    const gearKey = ['gear'] as const
    const listKeyA = ['list-items', 'A'] as const
    const listKeyB = ['list-items', 'B'] as const
    const listKeyC = ['list-items', 'C'] as const
    qc.setQueryData<Gear[]>(gearKey, [
      { id: 'g1', name: 'Tent', weight_grams: 1200 },
      { id: 'g2', name: 'Stove', weight_grams: 90 },
    ])
    // listKeyA references g1.
    qc.setQueryData<ListItem[]>(listKeyA, [
      { id: 'li1', gear_item_id: 'g1', quantity: 1, gear_item: { id: 'g1', name: 'Tent', weight_grams: 1200 } },
      { id: 'li2', gear_item_id: 'g2', quantity: 1, gear_item: { id: 'g2', name: 'Stove', weight_grams: 90 } },
    ])
    // listKeyB references g1 too (multi-list fan-out case).
    qc.setQueryData<ListItem[]>(listKeyB, [
      { id: 'li3', gear_item_id: 'g1', quantity: 2, gear_item: { id: 'g1', name: 'Tent', weight_grams: 1200 } },
    ])
    // listKeyC does NOT reference g1 — must be left untouched + uninvalidated.
    qc.setQueryData<ListItem[]>(listKeyC, [
      { id: 'li4', gear_item_id: 'g2', quantity: 1, gear_item: { id: 'g2', name: 'Stove', weight_grams: 90 } },
    ])
    return { qc, gearKey, listKeyA, listKeyB, listKeyC }
  }

  function build(qc: QueryClient, gearKey: readonly string[]) {
    return makeOptimisticUpdateWithFanout<Gear, ListItem, Patch>({
      qc,
      queryKey: gearKey,
      ...
      applyJoined: (item, { patch }) => ({ ...item, gear_item: { ...item.gear_item, ...patch } }),
    })
  }

  it('updates the primary cache', () => {
    const { qc, gearKey } = seed()
    const helper = build(qc, gearKey)
    helper.onMutate({ id: 'g1', patch: { name: 'Tent (lite)' } })
    expect(qc.getQueryData<Gear[]>(gearKey)).toEqual([
      { id: 'g1', name: 'Tent (lite)', weight_grams: 1200 },
      { id: 'g2', name: 'Stove', weight_grams: 90 },
    ])
  })

  it('updates every joined cache that contains a matching row', () => {
    const { qc, gearKey, listKeyA, listKeyB } = seed()
    const helper = build(qc, gearKey)
    helper.onMutate({ id: 'g1', patch: { name: 'Tent (lite)', weight_grams: 1100 } })
    // listKeyA has both g1 and g2 — only the g1-referencing row's embedded
    // gear should change; g2 row is untouched.
    expect(qc.getQueryData<ListItem[]>(listKeyA)).toEqual([
      { id: 'li1', gear_item_id: 'g1', quantity: 1, gear_item: { id: 'g1', name: 'Tent (lite)', weight_grams: 1100 } },
      { id: 'li2', gear_item_id: 'g2', quantity: 1, gear_item: { id: 'g2', name: 'Stove', weight_grams: 90 } },
    ])
    expect(qc.getQueryData<ListItem[]>(listKeyB)).toEqual([
      { id: 'li3', gear_item_id: 'g1', quantity: 2, gear_item: { id: 'g1', name: 'Tent (lite)', weight_grams: 1100 } },
    ])
  })

  it('leaves joined caches without a matching row untouched', () => {
    const { qc, gearKey, listKeyC } = seed()
    const before = qc.getQueryData<ListItem[]>(listKeyC)
    const helper = build(qc, gearKey)
    helper.onMutate({ id: 'g1', patch: { name: 'Tent (lite)' } })
    // listKeyC contained no row with gear_item_id === 'g1', so its data
    // reference should be the same as before — not a structurally-equal
    // clone. Same-reference proves the helper short-circuited.
    expect(qc.getQueryData<ListItem[]>(listKeyC)).toBe(before)
  })

  it('rolls back primary and every touched joined cache on error', () => {
    const { qc, gearKey, listKeyA, listKeyB, listKeyC } = seed()
    const gearBefore = qc.getQueryData<Gear[]>(gearKey)
    const aBefore = qc.getQueryData<ListItem[]>(listKeyA)
    const bBefore = qc.getQueryData<ListItem[]>(listKeyB)
    const cBefore = qc.getQueryData<ListItem[]>(listKeyC)
    const helper = build(qc, gearKey)
    const input: Patch = { id: 'g1', patch: { name: 'Tent (lite)' } }
    const ctx = helper.onMutate(input)
    helper.onError(new Error('boom'), input, ctx)
    expect(qc.getQueryData<Gear[]>(gearKey)).toEqual(gearBefore)
    expect(qc.getQueryData<ListItem[]>(listKeyA)).toEqual(aBefore)
    expect(qc.getQueryData<ListItem[]>(listKeyB)).toEqual(bBefore)
    // listKeyC never got written, so the rollback should also have
    // skipped it — but a structurally-equal compare is what we can
    // assert here (the snapshot list is the source of truth for which
    // caches participate in rollback).
    expect(qc.getQueryData<ListItem[]>(listKeyC)).toEqual(cBefore)
  })

  it('invalidates the primary key and only the joined keys it touched on settled', () => {
    const { qc, gearKey, listKeyA, listKeyB, listKeyC } = seed()
    ...
    // Primary always invalidated; A and B were touched and should be too.
    expect(invalidatedKeys).toContainEqual(gearKey)
    expect(invalidatedKeys).toContainEqual(listKeyA)
    expect(invalidatedKeys).toContainEqual(listKeyB)

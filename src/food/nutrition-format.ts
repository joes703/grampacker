...
// Nutrition target bands show one decimal with the trailing zero trimmed
// (30 -> "30", 30.5 -> "30.5"). Storage is numeric and the cells show decimals,
// so do NOT flatten fractional targets to integers with Math.round.
export function formatDailyTargetBand(metric: DailyTargetMetric, mode: TargetMode, min: number | null, max: number | null, unit: WeightUnit): string {
  ...
  return band(mode, min, max, (n) => `${trimNumber(n, 1)}${suffix}`)
}
export function formatMealTargetBand(metric: MealTargetMetric, mode: TargetMode, min: number | null, max: number | null): string {
  if (metric === 'fat_pct' || metric === 'sugar_pct') return band(mode, min, max, (n) => `${trimNumber(n, 1)}%`)

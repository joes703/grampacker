...
export default function PrintListHeader({ list, listItems, categories, breakdown }: Props) {
  return (
    <div className="hidden print:block">
      ...
      {(listItems.length > 0 || (breakdown?.totalPackGrams ?? 0) > 0) && (
        <div className="mt-3 mb-4">
          <WeightTable items={listItems} categories={categories} breakdown={breakdown} />
        </div>
      )}
    </div>
  )

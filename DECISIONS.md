...
**Date:** 2026-04-30 (migration `20260430000000_bulk_reorder_rpc.sql`); ownership check added 2026-05-01 (migration `20260501000000_bulk_reorder_rpc_ownership_check.sql`); converted to `SECURITY INVOKER` 2026-05-14 (migration `20260514202025_reduce_security_definer.sql`).
...
**Decision.** Bulk partial-column updates go through Postgres functions called via `supabase.rpc()`, not PostgREST upserts. `bulk_update_sort_order` is the canonical example: a fixed table whitelist (`categories`, `gear_items`, `lists`, `list_items`), `EXECUTE` revoked from `anon`, and an inline `user_id = auth.uid()` ownership filter per table branch. The `categories`, `gear_items`, and `lists` branches filter directly; the `list_items` branch joins `lists` and filters on `lists.user_id = auth.uid()`. It was originally `SECURITY DEFINER`; see "Security mode" below.
...
**Alternatives.** (a) Include every required column in the upsert payload. Rejected: whack-a-mole, since every new NOT NULL or RLS clause re-breaks the helper. (b) Switch the bulk helper back to `Promise.all` of single-row PATCHes. Rejected; that's the N-roundtrips bug we'd already fixed once. The RPC sidesteps the upsert problem entirely while keeping the bulk-write performance.

**Security mode (updated 2026-05-14).** The function was originally `SECURITY DEFINER`, which tripped Supabase's `authenticated_security_definer_function_executable` linter warning. Migration `20260514202025_reduce_security_definer.sql` converted it to `SECURITY INVOKER`, which resolves the warning rather than accepting it. The conversion is safe: the function only ever issues plain `UPDATE ... SET sort_order` statements (it became fully static in `20260501000000`, with no dynamic SQL and no INSERT path), so under INVOKER each branch's `UPDATE` is gated by the table's `*_auth_update` RLS policy, which enforces the same `auth.uid() = user_id` constraint the inline filter does. The inline ownership filters are kept as defense-in-depth and to preserve the silent-drop-on-non-owned-row behavior. `search_path` is tightened to `''` with every table reference schema-qualified. The original PostgREST-upsert failure mode lived in the auto-generated `INSERT ... ON CONFLICT` path, not in a plain `UPDATE`, so an INVOKER function issuing `UPDATE` sidesteps it the same way the DEFINER one did. DEFINER was not what made the RPC work. See the broader function audit in migration `20260514202025` (also converted `add_gear_item_with_list_item`, `create_list_from_selection`, and `duplicate_list`; `delete_account`, `handle_new_user`, and `rls_auto_enable` stay DEFINER because they genuinely need owner privileges).
...
**Consequences.** Row UI is denser, with two toggle slots per row, requiring tightened mobile layout. Weight summary is multi-row, not just a total. The `is_worn` and `is_consumable` columns are first-class on `list_items`, not derived.
...
**Alternatives.** (a) Account-wide public profile. Rejected; over-shares, since users curate which list they're proud of, not their whole closet. (b) No sharing. Rejected. Sharing for advice is a real use case (the FAQ on `/help` calls this out). (c) Authenticated-only sharing (recipient signs in to view). Rejected; adds friction the recipient often doesn't accept, and "send a link, no account needed" matches user expectations from Lighterpack and similar apps.
...
**Alternatives.**
- **(a) RLS WITH CHECK subquery against the FK target table.** Tried first (`20260506000000`). The `list_items_owner_all` policy's WITH CHECK clause was extended to add `EXISTS (SELECT 1 FROM gear_items WHERE gear_items.id = list_items.gear_item_id AND gear_items.user_id = auth.uid())`. **Rejected after applying.** The subquery against `gear_items` re-entered `gear_items`' own RLS policy, triggering Postgres `42P17` "infinite recursion detected in policy for relation list_items." Every `list_items` insert from the app failed silently. Reverted in `20260506000001`. **Rule learned: WITH CHECK subqueries that reference another RLS-protected table can trigger policy recursion, particularly when the referenced table's own policy needs to evaluate against the calling user (`auth.uid()`).** The specific failure mode here was `list_items`'s WITH CHECK doing `EXISTS (SELECT 1 FROM gear_items WHERE … AND gear_items.user_id = auth.uid())`, which forced gear_items' policy to evaluate, which (in some plans) re-entered list_items' evaluation. Schema-level constraints (composite FKs, BEFORE-INSERT triggers) avoid this by enforcing the invariant outside the policy layer.
- **(b) BEFORE INSERT/UPDATE trigger** on the child table that raises if the FK target's `user_id` doesn't match. Schema-light but trigger-heavy. Rejected; composite FKs are the canonical declarative form Postgres provides, and reaching for a trigger when a constraint suffices adds a control surface that's easier to forget when adding new FKs.

...
- When testing a DB helper, verify the production code path you think uses it actually does. Production callers can diverge from what a helper was "designed for" — e.g. the gear-items reorder uses `Promise.all` of single-row PATCHes, NOT `bulkUpdateSortOrder`. A passing test on table A tells you nothing about table B even if "they share a helper." Trace production callers manually and match tests to them; the bulk-reorder helper was silently broken for categories for weeks because the existing test exercised an unused path.

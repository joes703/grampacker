...
Owner-keyed tables carry one policy per (role, action) pair. Authenticated SELECT is owner-only: `(select auth.uid()) = user_id`. Public share reads go through curated security-invoker views plus column-level anon grants on only the public base columns those views need; private base columns remain ungranted to anon. Authenticated INSERT / UPDATE / DELETE are owner-only and gate on `(select auth.uid()) = user_id`; UPDATE additionally carries `WITH CHECK` against the same predicate so a user can't update one of their rows to belong to someone else. The `(select auth.uid())` form is a Supabase advisor recommendation: wrapping the function call in a subquery lets Postgres cache the value as an initPlan instead of re-evaluating it per row. The previous `*_owner_all` (FOR ALL) + `*_public_select_*` (FOR SELECT) pair-per-table shape was replaced in migration `20260512000000`; the public-share surface was moved to curated views in `20260615021614_public_gear_share_views` and converted to advisor-clean security-invoker views in `20260615023120_public_gear_share_invoker_views`.
...
| `categories` | `categories_auth_select`: `(select auth.uid()) = user_id` | `categories_auth_insert` / `categories_auth_update` / `categories_auth_delete`, all gated on `(select auth.uid()) = user_id` | — |
| `gear_items` | `gear_items_auth_select`: `(select auth.uid()) = user_id` | `gear_items_auth_insert` / `gear_items_auth_update` / `gear_items_auth_delete`, owner-keyed | — |
| `lists` | `lists_auth_select`: `(select auth.uid()) = user_id` | `lists_auth_insert` / `lists_auth_update` / `lists_auth_delete`, owner-keyed | — |
| `list_items` | `list_items_auth_select`: `(select auth.uid()) = user_id` (direct since `20260506000002` added `user_id`) | `list_items_auth_insert` / `list_items_auth_update` / `list_items_auth_delete`, owner-keyed | — |
...
Sharing is per-list and opt-in (see `DECISIONS.md` ADR 8 for the rationale). Each list has a 6-character `slug` generated at creation, and an `is_shared` boolean (default false). Public read access is granted to **`anon`** through curated views/RPCs, not raw base-table reads:

- `public_gear_lists`: shared list identity and presentation columns only.
- `public_gear_list_items`: shared list items plus flattened public gear fields.
- `public_gear_categories`: categories referenced by gear in shared lists.
- `food_projection_public`: aggregate Food-plan rows shown on shared Gear lists.
- `get_public_food_plan(slug)`: detailed Food-plan document, gated by both the shared Gear list and the plan's `is_food_shared` toggle.

The views are created with `security_barrier = true` and `security_invoker = true` to satisfy Supabase's security advisor. Because invoker views require base privileges, `anon` receives column-level SELECT grants only for the public base columns the views use. Private columns such as `list_items.is_packed`, `list_items.is_ready`, `gear_items.status`, `gear_items.cost`, `gear_items.purchase_date`, `food_items.notes`, and Food-plan entry ids are not granted to anon and cannot be requested through the Data API. The detailed Food-plan document intentionally does **not** use the invoker-view pattern: exposing day/meal/entry locations through an invoker view would require granting those base columns to anon, making them directly selectable even when `is_food_shared` is off. `get_public_food_plan` is a narrow `SECURITY DEFINER` RPC that performs the `slug + is_shared + is_food_shared` check internally and returns only its JSON allowlist. Authenticated non-owners do not get shared rows through base-table RLS. Signed-in users opening a friend's `/r/:slug` use a sessionless anonymous Supabase client for the public helpers, so the public route consistently evaluates through the anon public surfaces.

**Trust model:** the `slug` is a short, unguessable, public URL handle. Anyone with the URL can read the list while `is_shared = true`. We don't authenticate share-view requests (the slug IS the access mechanism), but we deliberately don't call it a credential because it isn't user-issued or password-like; it's a URL identifier in the same shape as a YouTube video ID or a Reddit post ID. Toggling `is_shared` off disables access without changing the slug; to break a leaked link, the user duplicates the list (which gets a fresh slug) and stops sharing the original. Slugs are 6 base62 characters (~57 billion combinations), so random guessing is impractical, but `public_gear_lists` exposes all currently shared lists to callers with Data API access to that view. We accept this: a shared list is by definition opt-in public-readable, and any user who doesn't want their list enumerable from the public surface leaves `is_shared` off. Unknown-slug requests at `/r/<slug>` 404 because of the route handler's not-found check, not because of any enumeration-defense logic in the view layer. See `SPEC.md` "Sharing mechanics" for the user-facing behavior.

**Public read column allowlist.** The database views physically omit owner-only metadata, and the client queries still use explicit column lists rather than `select('*')` as a second guard. The allowlist by view:

- **`public_gear_lists`** (via `fetchSharedList`): `id`, `slug`, `name`, `description`, `group_worn`, `is_draft`. The view also uses `is_shared` in its predicate, so anon has column-level SELECT on `is_shared`, but the view and client response do not expose it. Excluded from the view: `user_id`, `sort_order`, `ready_checks_enabled`, `created_at`, `updated_at`. `group_worn` and `is_draft` are deliberately public: the share view honors group-worn grouping and shows the draft banner.
- **`public_gear_list_items`** (via `fetchSharedListItems`): `id`, `list_id`, `gear_item_id`, `quantity`, `is_worn`, `is_consumable`, `sort_order`, `gear_name`, `gear_description`, `gear_weight_grams`, `gear_category_id`. The client selects everything except `list_id` because it already has the parent list's id. Excluded from the view: `is_packed` (owner's packing state, personal workflow data), `is_ready`, `user_id`, `created_at`, `updated_at`, and gear-private columns (`status`, `cost`, `purchase_date`, `sort_order`, `user_id`, timestamps).
- **`public_gear_categories`** (via `fetchSharedListCategories`): `id`, `name`, `sort_order`. Excluded from the view: `user_id`, `is_default`, `created_at`.
- **`food_projection_public`** (via `fetchSharedFoodProjection`): `list_slug`, `food_name`, `brand`, `total_effective_servings`, `total_weight_grams`. This is aggregate-only and gated by the parent Gear list's `is_shared`; it is intentionally independent of `food_plans.is_food_shared`, which gates the detailed Food-plan tab. Excluded from the view: `user_id`, relational ids, days, Meals, entry locations, nutrition columns, targets, `food_items.notes`, `food_pack_state`, timestamps, and `is_food_shared`.
- **`get_public_food_plan(slug)`** (via `fetchSharedFoodPlan`): one JSON document containing `plan.id/list_slug`, Meals (`id`, `name`, `anchor_role`, `is_default`, `sort_order`), days (`id`, `day_type_override`, `sort_order`), day-Meal cells (`id`, `day_id`, `meal_id`), entries (`id`, `day_meal_id`, `is_extra`, `food_item_id`, `basis`, `amount`, `sort_order`), used foods (label, serving, calories/nutrients, `sort_order`), and daily/Meal targets (`id`, metric, mode, bounds, plus `meal_id` for Meal targets). This is gated by both `lists.is_shared` and `food_plans.is_food_shared`; it is the detailed Food plan, so opaque relational ids and nutrition values are public-by-design while the toggle is on. Excluded from the RPC response: `user_id`, `list_id`, `food_plan_id`, `food_items.notes`, `food_pack_state`, timestamps, and all owner-only write metadata.

The `Public*` types in `src/lib/types.ts` (PublicList, PublicListItem, PublicGearItem, PublicCategory, PublicFoodProjection, PublicFoodPlanDocument) make the wire shape explicit at the type level. SharePage maps these narrow types to the full types at one boundary point so the shared rendering components can keep unified type signatures across authed and share-view contexts.

---

## SECURITY DEFINER functions

`SECURITY DEFINER` makes a Postgres function execute with the privileges of its owner (effectively a superuser, in Supabase's setup) instead of the calling user. This **bypasses RLS** for the duration of the function. We use it sparingly, and every one is structured to preserve the security boundary despite the bypass.

### Why we use it

`SECURITY DEFINER` is reserved for functions that act on objects the calling role provably cannot touch, or for tightly-shaped public RPCs where an invoker view would require unsafe base-column grants:
...
2. **`EXECUTE` revoked from `public` and unnecessary roles**, granted only to the role(s) that need the function. Owner-write definers grant to `authenticated`; public-read definers grant to `anon` and route browser reads through the sessionless public client; trigger-only functions grant to nobody.
3. **Inline authorization check** in the function body, *for callable definers*. Because RLS is bypassed, the function must re-assert the authorization rule itself. `delete_account` scopes the delete to `auth.uid()`. `get_public_food_plan` checks the public slug and both share toggles before reading plan data. Trigger-only definers (`handle_new_user`, `rls_auto_enable`) have no caller-supplied ids to check.
...
| `get_public_food_plan(slug)` | Public-read RPC; assembles the detailed Food-plan document without exposing detailed base columns directly to anon. Needs DEFINER because invoker views would require anon grants on day/Meal/entry base columns that must stay hidden when `is_food_shared` is off. | `anon` and `service_role` only. `authenticated` is intentionally not granted; browser public reads use the sessionless anon client. | Joins `lists` and `food_plans` by slug, then returns data only where `lists.is_shared = true` and `food_plans.is_food_shared = true`; otherwise returns `null`. | `20260615053852` |
...
`get_public_food_plan(slug)` is also `SECURITY DEFINER`, but it is not granted to `authenticated`; it is a public-read RPC granted to `anon` and used through the sessionless public client. If Supabase adds or raises a separate linter warning for anon-executable definers, keep the same bar: the function must need owner privileges, must set `search_path = ''`, must return only an allowlisted shape, and must perform its own public gating before reading data.
...
- **Query-level owner scoping.** Every private query helper (`fetchLists`, `fetchGearItems`, `fetchCategories`, `fetchListItems`) explicitly filters by `user_id = <auth uid>` even though RLS gates ownership. This is defense-in-depth and keeps query intent obvious. The earlier model also needed these filters to offset authenticated SELECT policies that matched own rows OR shared rows; that public-share branch was removed in `20260615021614_public_gear_share_views`. Public read paths use dedicated helpers (`fetchSharedList`, `fetchSharedListItems`, `fetchSharedListCategories`) that read curated public views through the sessionless anonymous Supabase client. New private query helpers must filter by `user_id`; new public helpers must read a curated view or RPC surface that physically omits private columns.
...
4. **Write policies** for SELECT/INSERT/UPDATE/DELETE. Use one policy per (role, action) pair. Two patterns:
   ...
   - **Joined-via-parent (child table without its own `user_id`):** swap each `(select auth.uid()) = user_id` predicate above with the joined form:
     ...
     ```
   - **Public-share read:** do not grant anon table-level SELECT and do not OR public predicates into authenticated base-table SELECT policies. Keep authenticated base-table access owner-only, expose the public shape through a curated view or RPC that physically omits private columns, and grant anon only the column-level base privileges required by any security-invoker view. Public browser helpers should use the sessionless anonymous Supabase client so signed-in public-link viewers follow the same path as signed-out viewers.
...
8. **If the table participates in sharing,** keep authenticated base-table access owner-only and create a curated public surface. Follow `public_gear_lists`, `public_gear_list_items`, `public_gear_categories`, and `food_projection_public`: `security_barrier = true`, `security_invoker = true`, explicit column list, no private columns, explicit grants on the view, and column-level anon grants only for the base columns the view needs.
9. **Do not rely on the legacy default public-schema grants for Data API access.** Every new public-schema table migration must include explicit grants from step 2 plus the RLS policies above. Never grant table privileges to the `public` role. Never grant anon table-level privileges on private base tables; for security-invoker public views, grant anon only the specific columns the view needs. Function `EXECUTE` grants follow the separate rules in "Adding a SECURITY DEFINER function safely" below.
...
| New public read path | Keep authenticated base-table access owner-only. Add a curated view/RPC with explicit columns, `security_barrier` where applicable, no private columns, and grants to the roles that need it. For security-invoker views, grant anon only the base columns required by the view and route public browser helpers through the sessionless anonymous Supabase client. Document the trust assumption (what gates it: slug + `is_shared` flag, public-by-design data, etc.). |
...
| Removing a public read path | Drop the curated public view/RPC and revoke its grants. Existing share links go silently 404, same as toggling `is_shared` off. |

...
Owner-keyed tables carry one policy per (role, action) pair. Authenticated SELECT is owner-only: `(select auth.uid()) = user_id`. Public share reads are not implemented with base-table anon policies; they go through curated security-barrier views that physically omit private columns. Authenticated INSERT / UPDATE / DELETE are owner-only and gate on `(select auth.uid()) = user_id`; UPDATE additionally carries `WITH CHECK` against the same predicate so a user can't update one of their rows to belong to someone else. The `(select auth.uid())` form is a Supabase advisor recommendation: wrapping the function call in a subquery lets Postgres cache the value as an initPlan instead of re-evaluating it per row. The previous `*_owner_all` (FOR ALL) + `*_public_select_*` (FOR SELECT) pair-per-table shape was replaced in migration `20260512000000`; the remaining public-share base-table policies were removed in `20260615021614_public_gear_share_views`.
...
- **`anon`**: unauthenticated. Used by the public share view (`/r/:slug`). Has no grants on the private base Gear-list tables. Reads public Gear data only through curated security-barrier views (`public_gear_lists`, `public_gear_list_items`, `public_gear_categories`). Has no write capability anywhere.
...
The views are created with `security_barrier = true` and `security_invoker = false`, so the caller needs only SELECT on the view and cannot request omitted base-table columns. The base tables (`lists`, `list_items`, `gear_items`, `categories`) have no anon grant, and authenticated non-owners do not get shared rows through base-table RLS. Signed-in users opening a friend's `/r/:slug` use the same views as signed-out users.
...
- **`public_gear_lists`** (via `fetchSharedList`): `id`, `slug`, `name`, `description`, `group_worn`, `is_draft`. The client selects everything except `slug` because it already has the slug from the URL. Excluded from the view: `user_id`, `is_shared`, `sort_order`, `ready_checks_enabled`, `created_at`, `updated_at`. `group_worn` and `is_draft` are deliberately public: the share view honors group-worn grouping and shows the draft banner.
...
4. **Write policies** for SELECT/INSERT/UPDATE/DELETE. Use one policy per (role, action) pair. Two patterns:
   ...
   - **Joined-via-parent (child table without its own `user_id`):** swap each `(select auth.uid()) = user_id` predicate above with the joined form:
     ...
     ```
   - **Public-share read:** do not add anon SELECT policies to private base tables and do not OR public predicates into authenticated base-table SELECT policies. Keep the base table owner-only, then expose the public shape through a curated view or RPC that physically omits private columns. Grant SELECT/EXECUTE on that public surface to `anon`, `authenticated`, and `service_role`.
...
8. **If the table participates in sharing,** keep the private table owner-only and create a curated public surface. Follow `public_gear_lists`, `public_gear_list_items`, and `public_gear_categories`: `security_barrier = true`, `security_invoker = false`, explicit column list, no private columns, and an explicit grant matrix on the view.
9. **Do not rely on the legacy default public-schema grants for Data API access.** Every new public-schema table migration must include explicit grants from step 2 plus the RLS policies above. Never grant table privileges to the `public` role. Never grant `anon` on private base tables; grant `anon` only on curated public views/RPCs. Function `EXECUTE` grants follow the separate rules in "Adding a SECURITY DEFINER function safely" below.
...
| New public read path | Keep base tables owner-only. Add a curated view/RPC with explicit columns, `security_barrier` where applicable, no private columns, and grants to `anon`, `authenticated`, and `service_role`. Document the trust assumption (what gates it: slug + `is_shared` flag, public-by-design data, etc.). |

...
> **Resolution status (updated 2026-06-07):** all 21 "C" findings are closed or resolved across PRs #29, #31, #32, #33, #34, #37, #38, #40. **C-11** is a deliberate WON'T FIX (shared status-badge color is intentional). **C-18** (AGENTS.md was a claude-mem dump) is resolved in #40 via a tracked stub + per-clone `skip-worktree` protection, after confirming the plugin cannot be redirected reliably per-project. **C-13** is resolved for the part that mattered (list-mutation/import consolidation); the within-category slice-based DnD remains accepted, documented tech debt. Per-finding RESOLVED notes are inline below.
...
- **RESOLVED (Stage 10, 2026-06-07):** Fixed via the atomic `create_list_with_imported_items` RPC (migration `20260607135857_atomic_list_import_rpc.sql`), `SECURITY INVOKER` + `search_path = ''` + inline `auth.uid()` check + RLS, modeled on `create_list_from_selection`. The TS planner (`src/lib/queries/import-plan.ts`) owns dedup/NFC/sort-order and emits a resolved plan of client-assigned UUIDs; the RPC validates ownership and every symbolic reference, then inserts categories -> gear -> list -> list_items in one transaction. Both consumers now route through `importListFromCsv` (`useListImportMutation` and `ListsEmptyState`); C-13's duplication was already collapsed in Stage 9, so the orphan path is closed in all copies. Covered by the pgTAP suite `supabase/tests/atomic_list_import.test.sql` (5 success + 1 failure + 4 per-table before/after rollback assertions) gated by the `DB Tests` CI workflow. **Atomicity caveat:** prevents partial writes; does NOT serialize dedup across concurrent imports (two simultaneous imports may each create the same "new" gear) - pre-existing, accepted behavior.
...
- **RESOLVED (PR #40):** Investigated redirecting claude-mem (option b) - not reliable per-project: the codex watch's `context.path` is a single global literal (a redirect would drop per-project `AGENTS.md` for every codex project), and `CLAUDE_MEM_EXCLUDED_PROJECTS` stops new capture but not the write (verified: `/api/context/inject` still served stored context after exclusion + worker restart). Fell back to option (c): `AGENTS.md` is now a tracked, stable stub pointing to `CLAUDE.md` and the authoritative docs; the local working copy is protected per-clone with `git update-index --skip-worktree AGENTS.md` so plugin overwrites never reach git. The stub and `.gitignore` document the (re-)apply/restore steps.

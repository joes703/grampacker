...
> **Resolution status (updated 2026-06-07):** 20 of 21 "C" findings closed across PRs #29, #31, #32, #33, #34, #37, #38. **C-11** is a deliberate WON'T FIX (shared status-badge color is intentional). **C-18** (AGENTS.md is a claude-mem dump) is the only item STILL OPEN - it needs an owner decision because the file is plugin-regenerated, not a normal code fix. **C-13** is resolved for the part that mattered (list-mutation/import consolidation); the within-category slice-based DnD remains accepted, documented tech debt. Per-finding RESOLVED notes are inline below.
...
- **STILL OPEN (2026-06-07) - needs owner decision:** `AGENTS.md` is still a `<claude-mem-context>` dump and is regenerated by the claude-mem plugin every session, so a one-off edit gets overwritten. This is not a normal code fix. Options: (a) `.gitignore` it and stop tracking; (b) reconfigure the plugin to write elsewhere; (c) replace with a committed stub redirecting to `CLAUDE.md` and exclude it from the plugin's output path. No PR yet.
